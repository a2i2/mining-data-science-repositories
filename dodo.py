...
DOCKER_VOLUME_PATH = ["--volume", "%s/input:/app/input" % CONFIG["volume_path"],
                      ...
                      "--volume", "%s/input_drive:/app/input_drive" % CONFIG["volume_path"],
...
def task_clone_non_data_science_projects():
    """Run the explore task for the project"""
    return {
        'actions': ["docker run --volume \"%s/\":/app %s python3 ./%s/task_clone_non_data_science_repos.py %s" % (CONFIG["volume_path"], IMAGE, PACKAGE_PATH, "%(args)s")],
        'params': PARAMS
    }
